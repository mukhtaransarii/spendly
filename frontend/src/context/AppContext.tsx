...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [authLoading, setAuthLoading] = useState(true) // true until token check done
  ...
  // On mount: verify token from localStorage before rendering anything
  useEffect(() => {
    const savedToken = localStorage.getItem('token')
    if (!savedToken) {
      setAuthLoading(false) // no token → show login immediately
      return
    }
    ...
    getMe()
      .then(me => {
        setUser({ _id: me._id, name: me.name, email: me.email })
        setToken(savedToken) // triggers data fetch below
      })
      .catch(() => {
        localStorage.removeItem('token')
        setAuthLoading(false) // invalid token → show login
      })
  }, [])

  // Load data once token is confirmed
  useEffect(() => {
    if (!token) return
    ...
    Promise.all([
      ...
      .finally(() => {
        setLoading(false)
        setAuthLoading(false) // done — show app, no flicker
      })
  }, [token])
